...
# Read data ----
world_data  <- readr::read_csv(file.path(data_path, "World.csv"))
kenya_data  <- readr::read_csv(file.path(data_path, "Kenya.csv"))
sweden_data <- readr::read_csv(file.path(data_path, "Sweden.csv"))
```
...
The crude birth rates have decreased in all three regions. 
...
kenya_data$asfr - sweden_data$asfr
```

The ASFR is consistently higher in Kenya than in Sweden. The fertility rate for women of the reproductive age is higher in Kenya. 
...
The number of women increased about $2.5$ times from $1950$ to $2000$. The total number of births in the world increased about $1.4$ times. 
...
# compute the ratio of CDR between the two periods
world_cdr[2] / world_cdr[1]
kenya_cdr[2] / kenya_cdr[1]
sweden_cdr[2] / sweden_cdr[1]
```

The CDR has drastically decreased in the world and Kenya. In Sweden, however, the CDR has not changed a lot.
...
# compare ASDR for Kenya and Sweden during the period of 2005-2010
kenya_data$asdr[16:30] / sweden_data$asdr[16:30]
```

The ASDR in Kenya was much higher than in Sweden in 2005. However, the ratio of ASDR between Kenya and Sweden has been decreasing in years.
...
mutate(kenya_data, temp_cdr = asdr * sweden_data$popP) %>%
  group_by(period) %>%
  summarise(cdrresweden = sum(temp_cdr)) -> kenya_cfcdr

# Compare the counterfactual CDR with the original CDR in Kenya
kenya_cfcdr$cdrresweden
kenya_cdr
```

The counterfactual CDR was higher than the original CDR. Especially in the 2005-2010 period, the counterfactual CDR is about 2.23 times greater thant he original CDR. 
